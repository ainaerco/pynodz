...
class Lagrange(object):
    def __init__(self, P, t):
        ...
        self._n = range(n)  # time/control point iterator
        self.t = t
...
def linspace(a, b, res, endpoint=False):
    result = [a]
...
def CatmullRom(p_x, p_y, res):
    ...
    x_intpol = [0.0] * (res * (len(p_x) - 1) + 1)
    y_intpol = [0.0] * (res * (len(p_x) - 1) + 1)
    ...
    for i in range(len(p_x) - 1):
        # set x-coords
        x_intpol[i * res : (i + 1) * res] = linspace(
            p_x[i], p_x[i + 1], res, endpoint=False
        )
        if i == 0:
            # need to estimate an additional support point before the first
            j = 0
            for x in linspace(0.0, 1.0, res, endpoint=False):
                y_intpol[j] = catmull_rom_one_point(
                    x,
            ...
            for x in linspace(0.0, 1.0, res, endpoint=False):
                y_intpol[j] = catmull_rom_one_point(
                    x, p_y[i - 1], p_y[i], p_y[i + 1], p_y[i + 2]
                )  # estimated end point
                j += 1
...
class Bspline(object):
    def __init__(self, P):
        ...
        t = k * [0.0] + [t_ * _t for t_ in range(m - (k * 2) + 1)] + [1.0] * k
        m, n = len(t) - 1, len(P) - 1
        if not k:
            k = m - n - 1
        else:
